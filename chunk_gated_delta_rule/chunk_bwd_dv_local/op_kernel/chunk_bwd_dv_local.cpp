                                                         ...
                                                         GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(1)) {
        GDN::VariableLengthStrategy variableStrategy{tilingData.chunkSize, tilingData.t, tilingData.chunkNumForT,cu_seqlens,chunk_indices};
        if ASCEND_IS_AIC {
                        GDN::ChunkBwdDvLocalCube<DTYPE_Q, DTYPE_G,GDN::VariableLengthStrategy> chunkBwdDvLocalCube(variableStrategy);
            chunkBwdDvLocalCube.Init(q, k, d_o, cu_seqlens, chunk_indices, d_v, userWS, &tilingData);
            chunkBwdDvLocalCube.Process();
        }
        if ASCEND_IS_AIV {
            AscendC::TPipe pipe;
                        GDN::ChunkBwdDvLocalVector<DTYPE_Q, DTYPE_G,GDN::VariableLengthStrategy> chunkBwdDvLocalVector(variableStrategy);
            chunkBwdDvLocalVector.Init(d_o, g, upper_tri_matrix, cu_seqlens, chunk_indices, d_v, userWS, &tilingData,
                                       &pipe);
            chunkBwdDvLocalVector.Process();
        }
